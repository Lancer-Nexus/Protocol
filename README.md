...
`ClientVersionHello` (keys 0–6) and `ClientVersionDecision` (keys 0–9) define the pre-login version exchange. Gateway owns the compatibility policy and issues the short-lived proof; the contract itself grants no permissions. `TransferTicketClaims` binds short-lived transfer admission to the session, character, source and target instances, target system and current lease version. Signing, replay handling and attachment checks remain service responsibilities.
